...
public class EletricidadeController {
    ...
    @GetMapping("/all")
    public ResponseEntity<List<EletricidadeModel>> listAllByUsuario(@RequestParam UUID usuario) {
        return ResponseEntity.status(HttpStatus.OK).body(eletricidadeService.listAllByUsuario(usuario));
    }
}
